...
public class ReportService {
    ...
    private final ClassService classService; // Added
    ...
    public List<Map<String, Object>> getClassPerformance(Long teacherId) {
        ...
        // Let's use ClassRepository directly if method exists or fetch all and filter
        ...
        for (ClassEntity cls : classes) {
            ...
            report.put("students", classService.getStudentsByClass(cls.getId()).size()); // Fixed

            // Calculate Avg Score (from ExamAttempts of students in this class)
            // This is heavy. Optimization: Cache or dedicated stats table.
            List<User> students = classService.getStudentsByClass(cls.getId()); // Fixed
            ...
            for (ClassSchedule sch : schedules) {
                ...
                presentSlots += atts.stream().filter(a -> a.getStatus() == Attendance.AttendanceStatus.PRESENT).count(); // Fixed
            }
    ...
    public List<Map<String, Object>> getTopStudents() {
        ...
        List<User> students = userService.getUsersByRole(User.UserRole.STUDENT); // Fixed
